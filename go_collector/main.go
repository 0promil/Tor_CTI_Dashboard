...
var (
	...
	dbUser     = getEnv("DB_USER", "0promil")
	dbPassword = getEnv("DB_PASSWORD", "0promil")
	dbName     = getEnv("DB_NAME", "0promil")
...
func onionAdresiniTara(onionURL string, onionID int, kaynakAdi string, db *sql.DB) ([]CTIKayit, error) {
	...
	os.WriteFile(filepath.Join(outDir, "site_data.html"), []byte(safeHTML), 0644)

	links := extractLinks(string(body))
	os.WriteFile(filepath.Join(outDir, "links.txt"), []byte(strings.Join(links, "\n")), 0644)
...
func kayitKaydet(db *sql.DB, kayit CTIKayit) error {
	query := `
		...
		(onion_id, kaynak_adi, kaynak_url, baslik, ozet, ana_kategori, alt_kategori, kategori, kritiklik, kritiklik_skor, kritiklik_aciklama)
...
func buildOutputDir(rawURL, title string) string {
	...
	return fmt.Sprintf("output/%s/%s_%s_%s", site, title, ts, short)
}
